...
/// Şu anda sayfada bulunan her kimse onun bilgilerini çeker */
    ...
    $postThumbnail = get_the_post_thumbnail_url(get_the_ID(),'full'); 
...
<article id="post-<?php the_ID(); ?>" <?php post_class('article-item '); ?>>
    ...
    <a class="image status" href="<?php the_permalink(); ?>" rel="bookmark" title="<?php the_title_attribute(); ?>">
        <?php if ( ! empty( $postThumbnail ) ) { ?>
            <img src="<?php echo $postThumbnail; ?>" class="attachment-post-thumbnail size-post-thumbnail wp-post-image" alt="" loading="lazy">
        <?php }
        the_excerpt(); ?>
    </a>
