        ...
        if (!already_liked($post_id, $is_comment)) { // Like the post
            if (is_user_logged_in()) { // user is logged in
                ...
                if ($is_comment == 1) {
                    ...
                    // Send Notification
                    $likedPost = get_post( $post_id );    // Beğenilen Yazı
                    $likedPostAuthorID = $likedPost->post_author; // Beğenilen Yazının Yazar ID

                    $authorNotifications = get_user_meta($likedPostAuthorID,'notifications',true);

                    // Yorum Yapınca Bildirim Gönder Yazı Sahibine
                    if ( $likedPostAuthorID !== $user_id && is_user_logged_in() ) { // Yazar - Kendi Yazısına Beğeni Atmıyorsa
                        if ( kelime_sor($authorNotifications, "liked#$user_id#$post_id")  == 1 ) { // Eğer Şuanki Kullanıcı Daha Önce Beğenmişse:  
                            // şu işlemleri yap
                        } else { 
            
                            // Add to Notifications
                            $authorNotifications .= "liked#$user_id#$post_id ";
                            update_user_meta( $likedPostAuthorID, 'notifications', $authorNotifications );
...
function get_liked_icon()
{
    ...
    $icon = '<span class="sl-icon"><svg class="icon"><use xlink:href="#icon-tea-liked" /></svg></span>';
    return $icon;
} // get_liked_icon()
...
function get_unliked_icon()
{
    ...
    $icon = '<span class="sl-icon"><svg class="icon"><use xlink:href="#icon-tea-unliked" /></svg></span>';
    return $icon;
} // get_unliked_icon()
